...
SELECT *, dense_rank() over(order by score desc) as rank_by_score
...
SELECT student_id, name, course, score,
    NTILE(4) OVER (PARTITION BY course ORDER BY score DESC) AS performance_group
FROM student_performance;

Question 6 : For each course, assign a row number based on attendance (highest first).

SELECT *, ROW_NUMBER() OVER (PARTITION BY course ORDER BY attendance DESC) AS attendance_rank
FROM student_performance;
...
SELECT student_id, name, REPLACE(city, 'Mumbai', 'MUM') AS city_display
FROM student_performance;

Question 10 : For each course, find the highest score using FIRST_VALUE().

SELECT student_id, name, course, score, FIRST_VALUE(score) OVER (PARTITION BY course ORDER BY score DESC) AS highest_score_in_course
FROM student_performance;
